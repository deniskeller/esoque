...
import { BaseButton, BaseInputApp, BaseSelect, BaseSelectApp } from "@base/index";
import { juristdictionData } from "@utils/juristdiction";
import styles from "./WidgetDarksideApp.module.scss";

const WidgetDarksideApp: React.FC<Props> = () => {
  ...
  const changeHandlerJuristdiction = (code: string) => {
    ...
    <>
      <div className={styles.Widget}>
        <div className={styles.Widgetform}>
          ...
          <BaseButton loading={loading} disabled={disabled} onClick={submitFormData} className={styles.Button}>
            New Request
          </BaseButton>
        </div>
                ...
                {error ? (
                  ...
                  <>
                    <p className={styles.NonExistentTitle}>
                      This Company Name <span>{response?.searched_company?.company_name || companyName}</span> is not
                      available
                    </p>
                    <p className={styles.NonExistentSubtitle}>{response?.message_description}</p>
                  </>
                )}
              </div>
            ) : (
              <div className={styles.Existent}>
                <div className={styles.ExistentHeader}>
                  <div className={styles.CompanyName}>
                    <div className={styles.Name}>{response?.company_name || companyName}</div>
                    <div className={styles.Title}>{response?.product_name || juristdiction}</div>
                    <div className={styles.Subtitle}>
                      {response?.product_description || "Government and Notary Fees / Registered Address"}
                    </div>
                  </div>
                  <div className={styles.CompanyPrice}>
                    <div className={styles.Title}>
                      Great, your company name: <span>{response?.company_name || companyName}</span> is free.
                    </div>
                    <p className={styles.Description}>{response?.message_description}</p>
...
export default WidgetDarksideApp;
