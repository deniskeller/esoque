...
const BaseSearchSelect: React.FC<Props> = ({
  ...
  return (
    <div className={`${styles.SelectContainer} ${styles["Select_" + type]} ${className}`} ref={selectContainerRef}>
      <div
        className={`${styles.SelectHeader}  ${isOpen ? styles.SelectHeaderFocus : ""} ${error ? styles.Error : ""}`}
        onClick={toggling}
      >
        <span className={`${selectedOption ? styles.NotEmpty : ""}`}>{selectedOption || placeholder || value}</span>
        <BaseIcon
      ...
      {isOpen && (
        <div className={styles.SelectList}>
          <div className={styles.SelectSearch}>
            <BaseIcon icon={ALL_ICONS.SEARCH} viewBox="0 0 34 34" fill="none" className={styles.IconSearch} />
            <input
              type="text"
          ...
          <ul className={styles.Ul}>
            {computedOptions(options).map((option: ISelectItem) => (
              <li
                className={styles.Li}
                onClick={onOptionClicked(option.code ? option.code! : option.title)}
                key={Math.random()}
              >
                {option.code && <span className={styles.CountryCode}>{option.code}</span>}
